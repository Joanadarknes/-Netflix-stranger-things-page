...
function cardPrincipal(temporada) {
    ...
    article.style.backgroundImage = `linear-gradient(to right,#000,transparent), url('${temporada.imagemURL}')`;
...
function cardTemporadaSelecionada(temporada) {
    ...
    imgTemporada.classList.add("iconCard")
    imgTemporada.style.backgroundImage = `url('${temporada.imagemIcone}')`;
    ...
    playlistCenter.insertAdjacentHTML("afterbegin", `
        ...
        <div class="playlist_li_playButton">
            <img src="./Projeto-netflix/assets/img/play-icon.png" />
...
playlistContainer.addEventListener("click", function(event) {
    const elementoHTML = event.target
    if (elementoHTML.tagName == "BUTTON") {
        const elementoArray = temporadasSelecionadas.find(temporada => elementoHTML.id == temporada.id)
        const indexElemento = temporadasSelecionadas.indexOf(elementoArray);
        temporadasSelecionadas.splice(indexElemento, 1);
